<?php
	...
	class modUnidadMedida {
		...
		public function cambiarEstado(unidadmedida $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_c_fisc_unidad_medida SET 
											estado			= :estado
									    WHERE id = :id "
									);
		...
		public function editar(unidadmedida $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
		     	...
		     	$consulta->bindValue(':abreviatura', $data->__GET('abreviatura'));
		...
		public function guardar(unidadmedida $data)
		{
			try 
			{
				...
				$consulta = $this->classConexion->getConexion()->
							prepare(
									"INSERT INTO cfg_c_fisc_unidad_medida (
															nombre, 
															abreviatura, 
															estado
														) 
		...
		public function consultar($id)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
					        prepare(
											" SELECT 
											    ...
											    estado as estadounidadmedida
											FROM    cfg_c_fisc_unidad_medida 
											WHERE id = :id "
					   				);
				...
				if ($r) {
					...
					$entidadResultados = new unidadmedida();

					$entidadResultados->__SET('id', $r->id);
					$entidadResultados->__SET('nombre', $r->nombre);
					$entidadResultados->__SET('abreviatura', $r->abreviatura);
					$entidadResultados->__SET('estado', $r->estadounidadmedida);
			...
			try
			{
				if(empty($filtro)) {
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT 
													...
													abreviatura as abreviatura,													
												    estado as estadounidadmedida
												FROM    cfg_c_fisc_unidad_medida as p 
				...
				foreach($consulta->fetchAll(PDO::FETCH_OBJ) as $r)
				{
					$result[] = array(
										...
										'estadounidadmedida' => $r->estadounidadmedida,
									);
				}
				$consulta = $this->classConexion->getCerrarSesion();
		...
		public function getTotalPaginas($filtro,$getTotalPaginas)
		{
			try
			{
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT COUNT(*) as num_filas FROM cfg_c_fisc_unidad_medida WHERE id LIKE :id ");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta

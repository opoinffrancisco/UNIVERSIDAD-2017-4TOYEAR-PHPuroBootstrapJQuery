<?php
	...
	class modDistribucion {
		...
		public function cambiarEstado(distribucion $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_c_logc_distribucion SET 
											estado			= :estado
									    WHERE id = :id "
									);
		...
		public function editar(distribucion $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
		...
		public function guardar(distribucion $data)
		{
			try 
			{
				...
				$consulta = $this->classConexion->getConexion()->
							prepare(
									"INSERT INTO cfg_c_logc_distribucion (
															nombre, 
															estado
														) 
						        				VALUES (
		...
		public function consultar($id)
		{
			try 
			{
				...
				if ($r) {
					...
					$entidadResultados = new distribucion();
				...
				if(empty($filtro)) {
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT 
													...
													nombre as nombre,
												    estado as estadoModelo
												FROM    cfg_c_logc_distribucion as p 
		...
		public function getTotalPaginas($filtro,$getTotalPaginas)
		{
			try
			{
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT COUNT(*) as num_filas FROM cfg_c_logc_distribucion WHERE id LIKE :id ");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta

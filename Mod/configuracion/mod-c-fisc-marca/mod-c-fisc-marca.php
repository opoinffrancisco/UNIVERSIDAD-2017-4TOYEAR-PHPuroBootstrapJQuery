<?php
	...
	class modMarca {
		...
		public function cambiarEstado(marca $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_c_fisc_mod_marca SET 
											estado			= :estado
									    WHERE id = :id "
									);
		...
		public function editar(marca $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
		...
		public function guardar(marca $data)
		{
			try 
			{
				...
				$consulta = $this->classConexion->getConexion()->
							prepare(
									"INSERT INTO cfg_c_fisc_mod_marca (
															nombre, 
															estado
														) 
						        				VALUES (
				...
				if(empty($filtro)) {
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT 
													...
													nombre as nombre,
												    estado as estadoModelo
												FROM    cfg_c_fisc_mod_marca as p 
		...
		public function getTotalPaginas($filtro,$getTotalPaginas)
		{
			try
			{
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT COUNT(*) as num_filas FROM cfg_c_fisc_mod_marca WHERE id LIKE :id ");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta

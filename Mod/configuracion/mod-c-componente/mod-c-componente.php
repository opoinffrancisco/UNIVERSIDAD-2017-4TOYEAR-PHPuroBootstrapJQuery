<?php
	...
	class modCaracteristicasComponentes extends caracteristicasComponentes{
		...
		public function cambiarEstado(caracteristicasComponentes $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_caracteristicas_fisc_comp SET 
											estado			= :estado
									    WHERE id = :id "
									);
		...
		public function editar(caracteristicasComponentes $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
									...
									"UPDATE cfg_caracteristicas_fisc_comp SET 
										...
										id_umcapacidad_fisc	  = :id_umcapacidad_fisc,
										capacidad		      = :capacidad
		     	...
		     	$consulta->bindValue(':id_umcapacidad_fisc', $data->__GET('id_umcapacidad_fisc'));
		     	$consulta->bindValue(':capacidad', $data->__GET('capacidad'));
		...
		public function guardar(caracteristicasComponentes $data)
		{
			try 
			{
				...
				$consulta = $this->classConexion->getConexion()->
							prepare(
									"INSERT INTO cfg_caracteristicas_fisc_comp (
															id_tipo_fisc, 
															id_modelo_fisc,
															id_umcapacidad_fisc,
															capacidad,
		...
		public function consultar($id)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
					        prepare(
										" SELECT
											...
											cfeq.id_tipo_fisc, 
											cfeq.id_modelo_fisc,
											cfeq.id_umcapacidad_fisc,
											cfeq.capacidad,											
											cfeq.estado,
                                            cfmm.id_marca  
										FROM  cfg_caracteristicas_fisc_comp as cfeq  
				...
				if ($r) {
					...
					$entidadResultados = new caracteristicasComponentes();
					$modelo = new modelo();
					$marca = new marca();
					...
					$entidadResultados->__SET('id_umcapacidad_fisc', $r->id_umcapacidad_fisc);
					$entidadResultados->__SET('capacidad', $r->capacidad);
				...
				if(empty($filtro) && empty($filtro2)) {
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT 
													...
													ccomp.estado as estado 
												FROM    cfg_caracteristicas_fisc_comp as ccomp 
													INNER JOIN cfg_c_fisc_tipo as cft ON ccomp.id_tipo_fisc = cft.id 										
													INNER JOIN cfg_c_fisc_modelo as cfm ON ccomp.id_modelo_fisc = cfm.id 
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT 
												...
												FROM    cfg_caracteristicas_fisc_comp as ccomp 
													...
													INNER JOIN cfg_c_fisc_modelo as cfm ON ccomp.id_modelo_fisc = cfm.id 
													INNER JOIN cfg_c_fisc_mod_marca as cfmm ON cfm.id_marca = cfmm.id  
												WHERE ccomp.id_tipo_fisc = :filtro2 
												ORDER BY ccomp.id DESC 
												LIMIT :empezardesde , :tamagnopaginas 
										");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT 
												...
												FROM    cfg_caracteristicas_fisc_comp as ccomp 
													...
													INNER JOIN cfg_c_fisc_modelo as cfm ON ccomp.id_modelo_fisc = cfm.id 
													INNER JOIN cfg_c_fisc_mod_marca as cfmm ON cfm.id_marca = cfmm.id  
												WHERE cfm.nombre LIKE :nombre AND ccomp.id_tipo_fisc = :filtro2 
												ORDER BY ccomp.id DESC 
												LIMIT :empezardesde , :tamagnopaginas 
										");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta
...
///////////////////////////////////////////////////////////////////////////////////////////////
		public function guardarInterfazComponente(interfazCaracteristicasComponentes $data)
		{
			...
			try 
			{
				...
				$consulta = $this->classConexion->getConexion()->
							prepare(
									"INSERT INTO cfg_interfaz_caracteristicas_fisicas_comp (
															id_interfaz, 
															id_c_fisc_comp,
															estado
														) 
		...
		public function ListarInterfacesComponente($idCaracteristicas)
		{ 
			$result = array();
			try
			{
				...
				$consulta = $this->classConexion->getConexion()->
							prepare("	SELECT 
										     ...
										     cfi.nombre,
										 	 icfq.estado,     
											 icfq.id_c_fisc_comp as idcComponentes  
										FROM `cfg_interfaz_caracteristicas_fisicas_comp` as icfq 
										INNER JOIN cfg_c_fisc_interfaz_conexion as cfi ON icfq.id_interfaz =  cfi.id 
										WHERE id_c_fisc_comp=:id_c_fisc_comp
									");



				// Gestionando parametros para LIMIT en el SQL
			   	$consulta->bindParam(':id_c_fisc_comp', $idCaracteristicas);			
				$consulta->execute();
				...
				foreach($consulta->fetchAll(PDO::FETCH_OBJ) as $r)
				{
					$result[] = array(
										...
										'idcComponentes' => $r->idcComponentes,
									);
				}
				$consulta = $this->classConexion->getCerrarSesion();
				return ($result) ? $result : 0 ;
		...
		public function cambiarEstadoInterfazComponente(interfazCaracteristicasComponentes $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_interfaz_caracteristicas_fisicas_comp SET 
											estado			= :estado
									    WHERE id_interfaz = :id_interfaz AND id_c_fisc_comp = :id_c_fisc_comp "
									);

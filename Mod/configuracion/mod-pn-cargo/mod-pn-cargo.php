<?php
	...
	class modCargo extends cargo{
		...
		public function cambiarEstado(cargo $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_pn_cargo SET 
											estado			= :estado
									    WHERE id = :id "
									);
		...
		public function editar(cargo $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
		     	...
		     	$consulta->bindValue(':responsable_departamento', $data->__GET('responsable_departamento'));
		...
		public function guardar(cargo $data)
		{
			try 
			{
		...
		public function consultar($id)
		{
			try 
			{
				...
				if ($r) {
					...
					$entidadResultados = new cargo();

					$entidadResultados->__SET('id', $r->id);
					$entidadResultados->__SET('nombre', $r->nombre);
					$entidadResultados->__SET('responsable_departamento', $r->responsable_departamento);
					$entidadResultados->__SET('estado', $r->estadoCargo);
		...
		public function getTotalPaginas($filtro,$getTotalPaginas)
		{
			try
			{
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT COUNT(*) as num_filas FROM cfg_pn_cargo WHERE id LIKE :id ");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta

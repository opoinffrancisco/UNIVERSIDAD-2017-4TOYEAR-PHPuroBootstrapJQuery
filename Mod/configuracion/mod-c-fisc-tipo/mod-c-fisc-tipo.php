<?php
	...
	class modTipo extends tipo{
		...
		public function cambiarEstado(tipo $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
							prepare(
										"UPDATE cfg_c_fisc_tipo SET 
											estado			= :estado
									    WHERE id = :id "
									);
		...
		public function editar(tipo $data)
		{
			try 
			{
				$consulta = $this->classConexion->getConexion()->
		...
		public function guardar(tipo $data)
		{
			try 
			{
				...
				$consulta = $this->classConexion->getConexion()->
					        prepare(
											" SELECT 
												id as id,
												nombre as nombre,
											    estado as estadoTipo
											FROM    cfg_c_fisc_tipo 
											WHERE id = :id "
					   				);
		...
		public function getTotalPaginas($filtro,$getTotalPaginas)
		{
			try
			{
					...
					$consulta = $this->classConexion->getConexion()->
								prepare("	 	SELECT COUNT(*) as num_filas FROM cfg_c_fisc_tipo WHERE id LIKE :id ");
					//se inserta el parametro de busqueda y se ejecuta lac onsulta

<?php
		...
		require_once '../../Mod/configuracion/mod-c-logc-distribucion/mod-c-logc-distribucion.php';
		//
		...
		*/
		...
		$modDistribucion = new modDistribucion();	
		//
			...
			if(isset($accion))
			{
				switch($accion)
				{
					case 'guardar':
						...
						$id=$_POST['id'];
						$resultados = $modDistribucion->consultar($id);
						if ($resultados!="") {
							/***********************EDITAR*************************/
							$distribucion->__SET('id',         $resultados->__GET('id'));
							$distribucion->__SET('nombre',       $_POST['nombre']);
							$resultado = $modDistribucion->editar($distribucion);
							if ($resultado===1) {

			                    $IP=$_POST['ip_cliente'];
			                    $IDUSUARIO=$_POST['id_usuario'];                                
			                    $modBitacora->guardarOperacion('EDITO UNA CARACTERISTICA LOGICA - DISTRIBUCION',$IP,$IDUSUARIO);

								$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡ Datos editados con Exito!",
													'detalles' => " distribucion editado correctamente ",												
												);								
							}else{
								$datos[] = array(
													'controlError' => 3,
													'mensaje' => "Error al modificar el distribucion",
													'detalles' => $resultado,												
												);
							}
							/**********************************************************/		
							...
							/**********************GUARDAR***********************/
							$distribucion->__SET('nombre',       $_POST['nombre']);
							$resultado =$modDistribucion->guardar($distribucion);
							if ($resultado>0) {

			                    $IP=$_POST['ip_cliente'];
			                    $IDUSUARIO=$_POST['id_usuario'];                                
			                    $modBitacora->guardarOperacion('GUARDO UNA CARACTERISTICA LOGICA - DISTRIBUCION',$IP,$IDUSUARIO);

								$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡Datos Guardados con Exito!",
													'detalles' => " distribucion Guardado correctamente ",												
												);								
							}else{
								$datos[] = array(
													'controlError' => 1,
													'mensaje' => "Error al Guardar el distribucion",
													'detalles' => $resultado,												
												);
							}
							/**********************************************************/	
					...
					*/
						...
						$distribucion->__SET('id',          $_POST['id']);
						$distribucion->__SET('estado',       $estado);						
						$resultadoModeloEstado = $modDistribucion->cambiarEstado($distribucion);
						if ($resultadoModeloEstado===1) {

		                    $IP=$_POST['ip_cliente'];
		                    $IDUSUARIO=$_POST['id_usuario'];                                
		                    $modBitacora->guardarOperacion($mensajeEstado3.' UNA CARACTERISTICA LOGICA - DISTRIBUCION',$IP,$IDUSUARIO);

							$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡Estado cambiado con Exito!",
													'detalles' => " distribucion ".$mensajeEstado." correctamente ",												
												);									
						}else{
							$datos[] = array(
												'controlError' => 4,
												'mensaje' => "Error al ".$mensajeEstado2." el distribucion",
					...
					case 'consultar':
						...
						if ($resultados!="") {
							$datos[] = array(
								...
								$datos[] = array(
													'controlError' => 2,
													'mensaje' => "No existe el distribucion con la id: ".$id,
												);
							}else{
								$datos[] = array(
													'controlError' => 2,
...
// INICIO CAlCULAR VARIABLES
						...
						$getTotalPaginas=1;
						$num_filas = $modDistribucion->getTotalPaginas($filtro,$getTotalPaginas);
						$total_paginas=ceil($num_filas/$tamagno_paginas);// ejemplo : (4) -> 1,2,3,4
// FIN CAlCULAR VARIABLES

						$getTotalPaginas=0;
						$resultados = $modDistribucion->Listar($filtro, $empezar_desde, $tamagno_paginas);
						...
						if ($resultados===0) {
							$datos = array(
												...
												'detalles' => "No Hay distribucion registrado",												
											);										
						}

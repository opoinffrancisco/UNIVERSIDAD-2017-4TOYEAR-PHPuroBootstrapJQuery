<?php
		...
		require_once '../../Mod/conexion.php';
	  	require_once '../../Mod/configuracion/mod-pn-cargo/entidad-pn-cargo.php';
		require_once '../../Mod/configuracion/mod-pn-cargo/mod-pn-cargo.php';
		//
			...
			if(isset($accion))
			{
				switch($accion)
				{
					case 'guardar':
						...
						$id=$_POST['id'];
						$resultados = $modCargo->consultar($id);
						if ($resultados!="") {
							/***********************EDITAR*************************/
							$cargo->__SET('id',         $resultados->__GET('id'));
							$cargo->__SET('nombre',       $_POST['nombre']);
							$cargo->__SET('responsable_departamento',       $_POST['responsable_departamento']);
							$resultado = $modCargo->editar($cargo);
							if ($resultado===1) {

								$IP=$_POST['ip_cliente'];
								$IDUSUARIO=$_POST['id_usuario'];								
								$modBitacora->guardarOperacion('EDITO UN CARGO',$IP,$IDUSUARIO);	

								$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡ Datos editados con Exito!",
													'detalles' => " cargo editado correctamente ",												
												);								
							}else{
								$datos[] = array(
													'controlError' => 3,
							...
							$cargo->__SET('nombre',       $_POST['nombre']);
							$cargo->__SET('responsable_departamento',       $_POST['responsable_departamento']);
							$resultado =$modCargo->guardar($cargo);
							if ($resultado>0) {

								$IP=$_POST['ip_cliente'];
								$IDUSUARIO=$_POST['id_usuario'];								
								$modBitacora->guardarOperacion('GUARDO UN CARGO',$IP,$IDUSUARIO);	

								$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡Datos Guardados con Exito!",
													'detalles' => " cargo Guardado correctamente ",												
												);								
							}else{
								$datos[] = array(
													'controlError' => 1,
					...
					*/
						...
						$cargo->__SET('estado',       $estado);						
						$resultadoDepartamentoEstado = $modCargo->cambiarEstado($cargo);
						if ($resultadoDepartamentoEstado===1) {

							$IP=$_POST['ip_cliente'];
							$IDUSUARIO=$_POST['id_usuario'];								
							$modBitacora->guardarOperacion($mensajeEstado3.' UN CARGO',$IP,$IDUSUARIO);

							$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡Estado cambiado con Exito!",
													'detalles' => " cargo ".$mensajeEstado." correctamente ",												
												);									
						}else{
							$datos[] = array(
												'controlError' => 4,
												'mensaje' => "Error al ".$mensajeEstado2." el cargo",
					...
					case 'consultar':
						...
						if ($resultados!="") {
							$datos[] = array(
								    ...
								    'responsable_departamento' => $resultados->__GET('responsable_departamento'),
							   		'estadoCargo' => $resultados->__GET('estado'),
									);

							} elseif($resultados===0) {
								$datos[] = array(
													'controlError' => 2,
													'mensaje' => "No existe el cargo con la id: ".$id,
												);
							}else{
								$datos[] = array(
													'controlError' => 2,
...
// INICIO CAlCULAR VARIABLES
						...
						$getTotalPaginas=1;
						$num_filas = $modCargo->getTotalPaginas($filtro,$getTotalPaginas);
						$total_paginas=ceil($num_filas/$tamagno_paginas);// ejemplo : (4) -> 1,2,3,4
// FIN CAlCULAR VARIABLES

						$getTotalPaginas=0;
						$resultados = $modCargo->Listar($filtro, $empezar_desde, $tamagno_paginas);
						...
						if ($resultados===0) {
							$datos = array(
												...
												'detalles' => "No Hay cargo registrada",												
											);										
						}

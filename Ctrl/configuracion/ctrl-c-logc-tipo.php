<?php
		...
		require_once '../../Mod/conexion.php';
	  	require_once '../../Mod/configuracion/mod-c-logc-tipo/entidad-c-logc-tipo.php';
		require_once '../../Mod/configuracion/mod-c-logc-tipo/mod-c-logc-tipo.php';
		//
				...
				switch($accion)
				{
					case 'guardar':
						...
						if ($resultados!="") {
							...
							if ($resultado===1) {
			                    ...
			                    $modBitacora->guardarOperacion('EDITO UNA CARACTERISTICA LOGICA - TIPO',$IP,$IDUSUARIO);

								$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡ Datos editados con Exito!",
													'detalles' => " Tipo de equipo editado correctamente ",												
												);								
							}else{
								$datos[] = array(
													'controlError' => 3,
													'mensaje' => "Error al modificar el Tipo de equipo",
													'detalles' => $resultado,												
												);
							}
							/**********************************************************/		
							...
							if ($resultado>0) {
			                    ...
			                    $IP=$_POST['ip_cliente'];
			                    $IDUSUARIO=$_POST['id_usuario'];                                
			                    $modBitacora->guardarOperacion('GUARDO UNA CARACTERISTICA LOGICA - TIPO',$IP,$IDUSUARIO);

								$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡Datos Guardados con Exito!",
													'detalles' => " Tipo de equipo Guardado correctamente ",												
												);								
							}else{
								$datos[] = array(
													'controlError' => 1,
													'mensaje' => "Error al Guardar el Tipo de equipo",
													'detalles' => $resultado,												
												);
							}
							/**********************************************************/	
					...
					*/
						...
						if ($resultadoModeloEstado===1) {
		                    ...
		                    $IP=$_POST['ip_cliente'];
		                    $IDUSUARIO=$_POST['id_usuario'];                                
		                    $modBitacora->guardarOperacion($mensajeEstado3.' UNA CARACTERISTICA LOGICA - TIPO',$IP,$IDUSUARIO);

							$datos[] = array(
													'controlError' => 0,
													'mensaje' => "¡Estado cambiado con Exito!",
													'detalles' => " Tipo de equipo ".$mensajeEstado." correctamente ",												
												);									
						}else{
							$datos[] = array(
												'controlError' => 4,
												'mensaje' => "Error al ".$mensajeEstado2." la Tipo de equipo",
					...
					case 'consultar':
						...
						if ($resultados!="") {
							$datos[] = array(
								...
								$datos[] = array(
													'controlError' => 2,
													'mensaje' => "No existe la Tipo de equipo con la id: ".$id,
												);
							}else{
								$datos[] = array(
													'controlError' => 2,
...
// FIN CAlCULAR VARIABLES
						...
						if ($resultados===0) {
							$datos = array(
												...
												'detalles' => "No Hay Tipo de equipo registrada",												
											);										
						}

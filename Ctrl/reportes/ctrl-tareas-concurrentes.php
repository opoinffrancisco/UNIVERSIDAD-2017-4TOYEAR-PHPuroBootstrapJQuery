			...
			{					
				case 'CFG-TAREAS-CONCURRENTES':
					...
					if (isset($_GET['datos'])) {
						...
						if (blickyerSIGMANSTEC::Check($datosEncriptados)) {
							...
							$resultados = $ModReportesProcesos->ListarRptTareasConcurrentes(
																				$marca_e,
																				$modelo_e,
																				$tipo_e,
																				$fecha_desde,
							...
							$PDFSIGMANSTEC->exportarPDF('tareas-concurrentes');
				...
				break;	
					case 'FiltrarLista':
						...
						$resultados = $modTareaProgramada->ListarReporteTareasConcurrentes($filtro_marca, $filtro_modelo, $filtro_tipo, $buscardordesde, $buscardorhasta);
						$datos = [	
										'controlError' => 0,						
										'resultados' => $resultados,
								];

	var GraficoMtdTareasConcurrentes = "";
	var mtdTareasConcurrentes = function () {
		...
		*/
	            ...
	            success:  function (data) {
	                ...
	                if (data.controlError==0) {
						...
						$(data).each(function (index, dato) {
							...
							GraficoMtdTareasConcurrentes.setData(dato.resultados);

						});
	
					}else{
							GraficoMtdTareasConcurrentes.setData([
		...
		var generarProcesoFiltrar = function() {
			...
			nucleo.getScreenshotOfElementPublic($("div#graficoRendimiento").get(0), 0, 0, ancho_div, alto_div, function(data) {
				...
				//----------------------------------------------------------
					...
					// Otros
					var _TITULO_ = "TAREAS CONCURRENTES";
					//
					...
					var win = window.open(configuracion.urlsPublic().modReporte.tabs.tareasConcurrentes.api+'?cfg='+_configuracion_+'&datos='+datosEncrypt);
					win.focus();
		...
		var iniciarPanelTabEquipos = function () {
				...
				mtdAsignar.filtrarSelectPublic();
			  	...
			  	mtdTareasConcurrentes.cargarListaDespegablePublic('tipoListD','cfg_c_fisc_tipo','un tipo');

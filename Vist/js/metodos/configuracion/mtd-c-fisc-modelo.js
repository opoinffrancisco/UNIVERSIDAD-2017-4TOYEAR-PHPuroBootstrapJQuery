var mtdCFiscModelo = function() {
	...
	*/
	/***************************Metodos de funcionalidades******************/	
		...
		*/
		var cargarCatalogo = function(_pagina_) {
			...
			var filtro_ = $("#ctlgCCompModelo #buscardorTxt").val();
	        ...
	        $.ajax({
				url: configuracion.urlsPublic().modCFisica.tabs.modelo.api,
				type:'POST',	
				data:{	
						accion:accion_,
						filtro:filtro_,
	            ...
	            beforeSend: function () {
			        ...
			        $("#ctlgCCompModelo #catalogoDatos").html('');				
			        $("#pgnCCompModelo #pagination").html('');	
					tr = $('<tr>');
					tr.append('<td colspan="5" style="text-align: center;"><img src="Vist/img/cargando2.gif" class="img-cargando"></td>');
					tr.append("</tr>");
					$('#ctlgCCompModelo #catalogoDatos').append(tr);
					...
					$("#btnNuevo").removeClass('imputSusess');
						...
						//Obteniendo resultados para catalogo
								...
								tr = $('<tr class="row '+ColorEstado+'">');
										...
										tr.append("<td style=' padding: 2px; text-align: center;vertical-align:middle;' class='col-md-4'>"+
												  "<div class='btn-group' role='group' style='width: 100%;'>"+
														  "<button type='button' class='btn btn-default editarBtnDiv' id='btnEditar' "+disabledEditar+" onclick='mtdCFiscModelo.consultarPublic("+datoItem.id+"),ventanaModal.mostrarBasicPublico()' style='width: 50%;'>Editar</button>"+
														  "<button type='button' class='btn btn-default eliminacionLogBtnDiv' onclick='mtdCFiscModelo.cambiarEstadoPublic("+datoItem.id+","+nuevoEstado+")'  style='width: 50%;'>"+AccionarEstado+"</button>"+
												  "</div>"+
												"</td>");
							...
							if (!(dato.pagActual<=1)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscModelo.controlPaginacionPublic('+dato.pagAnterior+')" aria-label="Previous">'+
								...
								if (dato.total_paginas>1) {
									...
									for (var i=1; i <= dato.total_paginas ; i++) { 
										...
										if(i==dato.pagActual) {
												ul.append(
															"<li class='active'>"+
																"<a href='JavaScript:;' onclick='mtdCFiscModelo.controlPaginacionPublic("+i+")' >"+ i +"</a>"+
															"</li>"
														); 
							...
							if (!(dato.pagActual>=dato.total_paginas)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscModelo.controlPaginacionPublic('+dato.pagSiguiente+')"  aria-label="Next">'+
							...
							$('#pgnCCompModelo #pagination').append(ul);
						});
						nucleo.asignarPermisosBotonesPublic(21);		
							...
							//-------------
							if (filtro_!=""){
								$("#ctlgCCompModelo #btnNuevo").prop('disabled', false);
								$("#ctlgCCompModelo #btnNuevo").addClass('imputSusess');
							}else{
								$("#ctlgCCompModelo #btnNuevo").prop('disabled', true);
								$("#ctlgCCompModelo #btnNuevo").removeClass('imputSusess');
							}
					}
	            },
		...
		*/	
		var consultar = function(id_departamento_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log(JSON.stringify(data));
					$('#vtnCCompModelo #datoControlId').val(data[0].id);
					$("#vtnCCompModelo #nombreTxt").val(data[0].nombre); 
					...
					$('#divMarcaTxt').popover('show');
				    setTimeout(function(){ 
						$("#vtnCCompModelo #marcaListD option[value='"+ data[0].id_marca+"']").attr("selected",true);	
		            	$('#divMarcaTxt').popover('destroy');
		...
		*/		
		var guardar = function() {
				...
				var id_marca_	= $("#marcaListD").val(); 
				var id_ = $('#vtnCCompModelo #datoControlId').val();
				//
		        $.ajax({
					url: configuracion.urlsPublic().modCFisica.tabs.modelo.api,
					type:'POST',	
	                ...
	                beforeSend: function () {
	                	$('#vtnCCompModelo #form').css("display", "none");
                        $('#vtnCCompModelo #procesandoDatosDialg').css("display", "block");
	                ...
	                success:  function (result) {
						...
						if (result[0].controlError==0) {
							...
							if ($('#ventana-modal-cfg').hasClass('ventana-modal-panel-accionMostrar')==true) {
								...
								mtdCFiscModelo.vaciarCatalogoPublic();
								mtdCFiscModelo.cargarCatalogoPublic(paginaActual);
							}
							return true;	                    
						}else{
		...
		*/		
		var cambiarEstado = function(id_,estado_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log("listo");
					mtdCFiscModelo.controlPaginacionPublic(paginaActual);
					return true;
	            },
		    	error: function(error) {
						console.log(JSON.stringify(error));
	...
	/****************************Metodos de control**************************/		
		...
		*/	
		var controlPaginacion = function (_pagina_) {
			...
			mtdCFiscModelo.cargarCatalogoPublic(_pagina_);   
		}	

		var activarEscuchador = function () {
		
			var idVtn = "#vtnCCompModelo";
			// -> Form Vtn
	...
	/****************Inicializacion e Intancias de metodos *******************/
		...
		return{
			Iniciar : function() {
				console.log("Iniciando carga de metodos de caracteristica del componente - Modelo");
					
					activarEscuchador();
				
				console.log("Finalizando carga de metodos de caracteristica del componente - Modelo");
			},

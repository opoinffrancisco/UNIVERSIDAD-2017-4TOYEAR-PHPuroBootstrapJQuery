var mtdCLogcTipo = function() {
	...
	*/
	/***************************Metodos de funcionalidades******************/	
		...
		*/
		var cargarCatalogo = function(_pagina_) {
			...
			var filtro_ = $("#ctlgCEqTipo #buscardorTxt").val();
	        ...
	        $.ajax({
				url: configuracion.urlsPublic().modCLogica.tabs.tipo.api,
				type:'POST',	
				data:{	
						accion:accion_,
						filtro:filtro_,
					...
					$("#btnNuevo").removeClass('imputSusess');
						...
						//Obteniendo resultados para catalogo
								...
								tr = $('<tr class="row '+ColorEstado+'">');
										...
										tr.append("<td style=' padding: 2px; text-align: center;vertical-align:middle;' class='col-md-4'>"+
												  "<div class='btn-group' role='group' style='width: 100%;'>"+
														  "<button type='button' class='btn btn-default editarBtnDiv' id='btnEditar' "+disabledEditar+" onclick='mtdCLogcTipo.consultarPublic("+datoItem.id+"),ventanaModal.mostrarBasicPublico()' style='width: 50%;'>Editar</button>"+
														  "<button type='button' class='btn btn-default eliminacionLogBtnDiv' onclick='mtdCLogcTipo.cambiarEstadoPublic("+datoItem.id+","+nuevoEstado+")'  style='width: 50%;'>"+AccionarEstado+"</button>"+
												  "</div>"+
												"</td>");
							...
							if (!(dato.pagActual<=1)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCLogcTipo.controlPaginacionPublic('+dato.pagAnterior+')" aria-label="Previous">'+
								...
								if (dato.total_paginas>1) {
									...
									for (var i=1; i <= dato.total_paginas ; i++) { 
										...
										if(i==dato.pagActual) {
												ul.append(
															"<li class='active'>"+
																"<a href='JavaScript:;' onclick='mtdCLogcTipo.controlPaginacionPublic("+i+")' >"+ i +"</a>"+
															"</li>"
														); 
							...
							if (!(dato.pagActual>=dato.total_paginas)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCLogcTipo.controlPaginacionPublic('+dato.pagSiguiente+')"  aria-label="Next">'+
							...
							$('#pgnCEqTipo #pagination').append(ul);
						});
						nucleo.asignarPermisosBotonesPublic(17);
							...
							//-------------
							if (filtro_!=""){
								$("#ctlgCEqTipo #btnNuevo").prop('disabled', false);
								$("#ctlgCEqTipo #btnNuevo").addClass('imputSusess');
							}else{
								$("#ctlgCEqTipo #btnNuevo").prop('disabled', true);
								$("#ctlgCEqTipo #btnNuevo").removeClass('imputSusess');
							}
					}
	            },
		...
		*/	
		var consultar = function(id_departamento_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log(JSON.stringify(data));
					$('#vtnCEqTipo #datoControlId').val(data[0].id);
					$("#vtnCEqTipo #nombreTxt").val(data[0].nombre); 
	            },
		...
		*/		
		var guardar = function() {
				...
				var id_ = $('#vtnCEqTipo #datoControlId').val();
				//
		        $.ajax({
					url: configuracion.urlsPublic().modCLogica.tabs.tipo.api,
					type:'POST',	
	                ...
	                beforeSend: function () {
	                	$('#vtnCEqTipo #form').css("display", "none");
                        $('#vtnCEqTipo #procesandoDatosDialg').css("display", "block");
	                ...
	                success:  function (result) {
						...
						if (result[0].controlError==0) {
							...
							if ($('#ventana-modal-cfg').hasClass('ventana-modal-panel-accionMostrar')==true) {
								...
								mtdCLogcTipo.vaciarCatalogoPublic();
								mtdCLogcTipo.cargarCatalogoPublic(paginaActual);
							}
							return true;	                    
						}else{
		...
		*/		
		var cambiarEstado = function(id_,estado_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log("listo");
					mtdCLogcTipo.controlPaginacionPublic(paginaActual);
					return true;
	            },
		    	error: function(error) {
						console.log(JSON.stringify(error));
	...
	/****************************Metodos de control**************************/		
		...
		*/	
		var controlPaginacion = function (_pagina_) {
			...
			mtdCLogcTipo.cargarCatalogoPublic(_pagina_);   
		}	
		var activarEscuchador = function () {
		
			var idVtn = "#vtnCEqTipo";
			// -> Form Vtn
	...
	/****************Inicializacion e Intancias de metodos *******************/
		...
		return{
			Iniciar : function() {
				console.log("Iniciando carga de metodos de caracteristica del equipo - Tipo de equipo");
				
				activarEscuchador();
				console.log("Finalizando carga de metodos de caracteristica del equipo - Tipo de equipo");
			},

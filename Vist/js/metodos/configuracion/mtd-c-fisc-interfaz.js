var mtdCFiscInterfaz = function() {
	...
	*/
	/***************************Metodos de funcionalidades******************/	
		...
		*/
		var cargarCatalogo = function(_pagina_) {
			...
			var filtro_ = $("#ctlgCCompInterfaz #buscardorTxt").val();
	        ...
	        $.ajax({
				url: configuracion.urlsPublic().modCFisica.tabs.interfaz.api,
				type:'POST',	
				data:{	
						accion:accion_,
						filtro:filtro_,
	            ...
	            beforeSend: function () {
			        ...
			        $("#ctlgCCompInterfaz #catalogoDatos").html('');				
			        $("#pgnCCompInterfaz #pagination").html('');	
					tr = $('<tr>');
					tr.append('<td colspan="5" style="text-align: center;"><img src="Vist/img/cargando2.gif" class="img-cargando"></td>');
					tr.append("</tr>");
					$('#ctlgCCompInterfaz #catalogoDatos').append(tr);
					...
					$("#btnNuevo").removeClass('imputSusess');
						...
						//Obteniendo resultados para catalogo
								...
								tr = $('<tr class="row '+ColorEstado+'">');
										...
										tr.append("<td style=' padding: 2px; text-align: center;vertical-align:middle;' class='col-md-4'>"+
												  "<div class='btn-group' role='group' style='width: 100%;'>"+
														  "<button type='button' class='btn btn-default editarBtnDiv' id='btnEditar' "+disabledEditar+" onclick='mtdCFiscInterfaz.consultarPublic("+datoItem.id+"),ventanaModal.mostrarBasicPublico()' style='width: 50%;'>Editar</button>"+
														  "<button type='button' class='btn btn-default eliminacionLogBtnDiv' onclick='mtdCFiscInterfaz.cambiarEstadoPublic("+datoItem.id+","+nuevoEstado+")'  style='width: 50%;'>"+AccionarEstado+"</button>"+
												  "</div>"+
												"</td>");
							...
							if (!(dato.pagActual<=1)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscInterfaz.controlPaginacionPublic('+dato.pagAnterior+')" aria-label="Previous">'+
								...
								if (dato.total_paginas>1) {
									...
									for (var i=1; i <= dato.total_paginas ; i++) { 
										...
										if(i==dato.pagActual) {
												ul.append(
															"<li class='active'>"+
																"<a href='JavaScript:;' onclick='mtdCFiscInterfaz.controlPaginacionPublic("+i+")' >"+ i +"</a>"+
															"</li>"
														); 
							...
							if (!(dato.pagActual>=dato.total_paginas)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscInterfaz.controlPaginacionPublic('+dato.pagSiguiente+')"  aria-label="Next">'+
							...
							$('#pgnCCompInterfaz #pagination').append(ul);
						});
						nucleo.asignarPermisosBotonesPublic(22);
					}else{
							tr = $('<tr>');
							...
							//-------------
							if (filtro_!=""){
								$("#ctlgCCompInterfaz #btnNuevo").prop('disabled', false);
								$("#ctlgCCompInterfaz #btnNuevo").addClass('imputSusess');
							}else{
								$("#ctlgCCompInterfaz #btnNuevo").prop('disabled', true);
								$("#ctlgCCompInterfaz #btnNuevo").removeClass('imputSusess');
							}
					}
	            },
		...
		*/	
		var consultar = function(id_departamento_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log(JSON.stringify(data));
					$('#vtnCCompInterfaz #datoControlId').val(data[0].id);
					$("#vtnCCompInterfaz #nombreTxt").val(data[0].nombre); 
		...
		*/		
		var guardar = function() {
				...
				var id_ = $('#vtnCCompInterfaz #datoControlId').val();
				//
		        $.ajax({
					url: configuracion.urlsPublic().modCFisica.tabs.interfaz.api,
					type:'POST',	
	                ...
	                beforeSend: function () {
	                	$('#vtnCCompInterfaz #form').css("display", "none");
                        $('#vtnCCompInterfaz #procesandoDatosDialg').css("display", "block");
	                ...
	                success:  function (result) {
						...
						if (result[0].controlError==0) {
							...
							if ($('#ventana-modal-cfg').hasClass('ventana-modal-panel-accionMostrar')==true) {
								...
								mtdCFiscInterfaz.vaciarCatalogoPublic();
								mtdCFiscInterfaz.cargarCatalogoPublic(paginaActual);
							}
							return true;	                    
						}else{
		...
		*/		
		var cambiarEstado = function(id_,estado_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log("listo");
					mtdCFiscInterfaz.controlPaginacionPublic(paginaActual);
					return true;
	            },
		    	error: function(error) {
						console.log(JSON.stringify(error));
	...
	/****************************Metodos de control**************************/		
		...
		*/	
		var controlPaginacion = function (_pagina_) {
			...
			mtdCFiscInterfaz.cargarCatalogoPublic(_pagina_);   
		}	

		var activarEscuchador = function () {
		
			var idVtn = "#vtnCCompInterfaz";
			// -> Form Vtn
	...
	/****************Inicializacion e Intancias de metodos *******************/
		...
		return{
			Iniciar : function() {
				console.log("Iniciando carga de metodos de caracteristica del componente - Interfaz");
				
					activarEscuchador();
				
				console.log("Finalizando carga de metodos de caracteristica del componente - Interfaz");
			},

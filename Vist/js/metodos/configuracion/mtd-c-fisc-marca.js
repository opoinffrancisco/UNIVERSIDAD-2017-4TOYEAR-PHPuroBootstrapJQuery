var mtdCFiscMarca = function() {
	...
	*/
	/***************************Metodos de funcionalidades******************/	
		...
		*/
		var cargarCatalogo = function(_pagina_) {
			...
			var filtro_ = $("#ctlgCCompMarca #buscardorTxt").val();
	        ...
	        $.ajax({
				url: configuracion.urlsPublic().modCFisica.tabs.marca.api,
				type:'POST',	
				data:{	
						accion:accion_,
						filtro:filtro_,
	            ...
	            beforeSend: function () {
			        ...
			        $("#ctlgCCompMarca #catalogoDatos").html('');				
			        $("#pgnCCompMarca #pagination").html('');	
					tr = $('<tr>');
					tr.append('<td colspan="5" style="text-align: center;"><img src="Vist/img/cargando2.gif" class="img-cargando"></td>');
					tr.append("</tr>");
					$('#ctlgCCompMarca #catalogoDatos').append(tr);
					...
					$("#btnNuevo").removeClass('imputSusess');
						...
						//Obteniendo resultados para catalogo
								...
								tr = $('<tr class="row '+ColorEstado+'">');
										...
										tr.append("<td style=' padding: 2px; text-align: center;vertical-align:middle;' class='col-md-4'>"+
												  "<div class='btn-group' role='group' style='width: 100%;'>"+
														  "<button type='button' class='btn btn-default editarBtnDiv' id='btnEditar' "+disabledEditar+" onclick='mtdCFiscMarca.consultarPublic("+datoItem.id+"),ventanaModal.mostrarBasicPublico()' style='width: 50%;'>Editar</button>"+
														  "<button type='button' class='btn btn-default eliminacionLogBtnDiv' onclick='mtdCFiscMarca.cambiarEstadoPublic("+datoItem.id+","+nuevoEstado+")'  style='width: 50%;'>"+AccionarEstado+"</button>"+
												  "</div>"+
												"</td>");
							...
							if (!(dato.pagActual<=1)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscMarca.controlPaginacionPublic('+dato.pagAnterior+')" aria-label="Previous">'+
								...
								if (dato.total_paginas>1) {
									...
									for (var i=1; i <= dato.total_paginas ; i++) { 
										...
										if(i==dato.pagActual) {
												ul.append(
															"<li class='active'>"+
																"<a href='JavaScript:;' onclick='mtdCFiscMarca.controlPaginacionPublic("+i+")' >"+ i +"</a>"+
															"</li>"
														); 
							...
							if (!(dato.pagActual>=dato.total_paginas)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscMarca.controlPaginacionPublic('+dato.pagSiguiente+')"  aria-label="Next">'+
							...
							$('#pgnCCompMarca #pagination').append(ul);
						});
							...
							//-------------
							if (filtro_!=""){
								$("#ctlgCCompMarca #btnNuevo").prop('disabled', false);
								$("#ctlgCCompMarca #btnNuevo").addClass('imputSusess');
							}else{
								$("#ctlgCCompMarca #btnNuevo").prop('disabled', true);
								$("#ctlgCCompMarca #btnNuevo").removeClass('imputSusess');
							}
					}
	            },
		...
		*/	
		var consultar = function(id_departamento_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log(JSON.stringify(data));
					$('#vtnCCompMarca #datoControlId').val(data[0].id);
					$("#vtnCCompMarca #nombreTxt").val(data[0].nombre); 
		...
		*/		
		var guardar = function() {
				...
				var id_ = $('#vtnCCompMarca #datoControlId').val();
				//
		        $.ajax({
					url: configuracion.urlsPublic().modCFisica.tabs.marca.api,
					type:'POST',	
	                ...
	                beforeSend: function () {
	                	$('#vtnCCompMarca #form').css("display", "none");
                        $('#vtnCCompMarca #procesandoDatosDialg').css("display", "block");
	                ...
	                success:  function (result) {
						...
						if (result[0].controlError==0) {
							...
							if ($('#ventana-modal-cfg').hasClass('ventana-modal-panel-accionMostrar')==true) {
								...
								mtdCFiscMarca.vaciarCatalogoPublic();
								mtdCFiscMarca.cargarCatalogoPublic(paginaActual);
							}
							return true;	                    
						}else{
		...
		*/		
		var cambiarEstado = function(id_,estado_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log("listo");
					mtdCFiscMarca.controlPaginacionPublic(paginaActual);
					return true;
	            },
		    	error: function(error) {
						console.log(JSON.stringify(error));
	...
	/****************************Metodos de control**************************/		
		...
		*/	
		var controlPaginacion = function (_pagina_) {
			...
			mtdCFiscMarca.cargarCatalogoPublic(_pagina_);   
		}	

		var activarEscuchador = function () {
		
			var idVtn = "#vtnCCompMarca";
			// -> Form Vtn
	...
	/****************Inicializacion e Intancias de metodos *******************/
		...
		return{
			Iniciar : function() {
				console.log("Iniciando carga de metodos de caracteristica del componente - Marca");
					
					activarEscuchador();

				console.log("Finalizando carga de metodos de caracteristica del componente - Marca");
			},

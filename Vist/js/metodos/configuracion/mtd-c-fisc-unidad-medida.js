var mtdCFiscUMCapacidad = function() {
	...
	*/
	/***************************Metodos de funcionalidades******************/	
		...
		*/
		var cargarCatalogo = function(_pagina_) {
			...
			var filtro_ = $("#ctlgCCompUMCapacidad #buscardorTxt").val();
	        ...
	        $.ajax({
				url: configuracion.urlsPublic().modCFisica.tabs.umCapacidad.api,
				type:'POST',	
				data:{	
						accion:accion_,
						filtro:filtro_,
	            ...
	            beforeSend: function () {
			        ...
			        $("#ctlgCCompUMCapacidad #catalogoDatos").html('');				
			        $("#pgnCCompUMCapacidad #pagination").html('');	
					tr = $('<tr>');
					tr.append('<td colspan="5" style="text-align: center;"><img src="Vist/img/cargando2.gif" class="img-cargando"></td>');
					tr.append("</tr>");
					$('#ctlgCCompUMCapacidad #catalogoDatos').append(tr);
					...
					$("#btnNuevo").removeClass('imputSusess');
						...
						//Obteniendo resultados para catalogo
		    				$(dato.resultados).each(function (index, datoItem) {                				   
								if (datoItem.estadounidadmedida==1) {
								...
								tr = $('<tr class="row '+ColorEstado+'">');
										...
										tr.append("<td style=' padding: 0px; text-align: center;vertical-align:middle;' class='col-md-2'>" + datoItem.abreviatura + "</td>");
										...
										tr.append("<td style=' padding: 2px; text-align: center;vertical-align:middle;' class='col-md-4'>"+
												  "<div class='btn-group' role='group' style='width: 100%;'>"+
														  "<button type='button' class='btn btn-default editarBtnDiv' id='btnEditar' "+disabledEditar+" onclick='mtdCFiscUMCapacidad.consultarPublic("+datoItem.id+"),ventanaModal.mostrarBasicPublico()' style='width: 50%;'>Editar</button>"+
														  "<button type='button' class='btn btn-default eliminacionLogBtnDiv' onclick='mtdCFiscUMCapacidad.cambiarEstadoPublic("+datoItem.id+","+nuevoEstado+")'  style='width: 50%;'>"+AccionarEstado+"</button>"+
												  "</div>"+
												"</td>");
							...
							if (!(dato.pagActual<=1)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscUMCapacidad.controlPaginacionPublic('+dato.pagAnterior+')" aria-label="Previous">'+
								...
								if (dato.total_paginas>1) {
									...
									for (var i=1; i <= dato.total_paginas ; i++) { 
										...
										if(i==dato.pagActual) {
												ul.append(
															"<li class='active'>"+
																"<a href='JavaScript:;' onclick='mtdCFiscUMCapacidad.controlPaginacionPublic("+i+")' >"+ i +"</a>"+
															"</li>"
														); 
							...
							if (!(dato.pagActual>=dato.total_paginas)) {
								ul.append(
											'<li>'+
									 			'<a href="JavaScript:;" onclick="mtdCFiscUMCapacidad.controlPaginacionPublic('+dato.pagSiguiente+')"  aria-label="Next">'+
							...
							$('#pgnCCompUMCapacidad #pagination').append(ul);
						});
						nucleo.asignarPermisosBotonesPublic(23);
							...
							//-------------
							if (filtro_!=""){
								$("#ctlgCCompUMCapacidad #btnNuevo").prop('disabled', false);
								$("#ctlgCCompUMCapacidad #btnNuevo").addClass('imputSusess');
							}else{
								$("#ctlgCCompUMCapacidad #btnNuevo").prop('disabled', true);
								$("#ctlgCCompUMCapacidad #btnNuevo").removeClass('imputSusess');
							}
					}
	            },
		...
		*/	
		var consultar = function(id_departamento_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log(JSON.stringify(data));
					$('#vtnCCompUMCapacidad #datoControlId').val(data[0].id);
					$("#vtnCCompUMCapacidad #nombreTxt").val(data[0].nombre); 
					$("#vtnCCompUMCapacidad #abreviaturaTxt").val(data[0].abreviatura); 
		...
		*/		
		var guardar = function() {
				...
				var abreviatura_ = $("#abreviaturaTxt").val(); 

				var id_ = $('#vtnCCompUMCapacidad #datoControlId').val();
				//
		        $.ajax({
					url: configuracion.urlsPublic().modCFisica.tabs.umCapacidad.api,
					type:'POST',	
	                ...
	                beforeSend: function () {
	                	$('#vtnCCompUMCapacidad #form').css("display", "none");
                        $('#vtnCCompUMCapacidad #procesandoDatosDialg').css("display", "block");
	                ...
	                success:  function (result) {
						...
						if (result[0].controlError==0) {
							...
							if ($('#ventana-modal-cfg').hasClass('ventana-modal-panel-accionMostrar')==true) {
								...
								mtdCFiscUMCapacidad.vaciarCatalogoPublic();
								mtdCFiscUMCapacidad.cargarCatalogoPublic(paginaActual);
							}
							return true;	                    
						}else{
		...
		*/		
		var cambiarEstado = function(id_,estado_) {
	        ...
	        $.ajax({
	            ...
	            success:  function (data) {
					console.log("listo");
					mtdCFiscUMCapacidad.controlPaginacionPublic(paginaActual);
					return true;
	            },
		    	error: function(error) {
						console.log(JSON.stringify(error));
	...
	/****************************Metodos de control**************************/		
		...
		*/
		var restablecerForm = function() {
			//
	        $('#vtnCCompUMCapacidad #form')[0].reset();
		...
		*/	
		var controlPaginacion = function (_pagina_) {
			...
			mtdCFiscUMCapacidad.cargarCatalogoPublic(_pagina_);   
		}	

		var activarEscuchador = function () {
		
			var idVtn = "#vtnCCompUMCapacidad";
			// -> Form Vtn
	...
	/****************Inicializacion e Intancias de metodos *******************/
		...
		return{
			Iniciar : function() {
				console.log("Iniciando carga de metodos de caracteristica del Componente - Unidad de medida de la Capacidad");

					activarEscuchador();				
					
				console.log("Finalizando carga de metodos de caracteristica del Componente - Unidad de medida de la Capacidad");
			},
